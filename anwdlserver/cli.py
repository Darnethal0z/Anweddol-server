...
server management commands:
  dl-iso      download a copy of the official container ISO
...
Sends a SIGINT signal to the server daemon to stop it.""",
    ...
    def dl_iso(self):
        parser = argparse.ArgumentParser(
            formatter_class=argparse.RawDescriptionHelpFormatter,
            description="""\033[1m-> Download a copy of the official container ISO\033[0m

If no mirror URL is specified, will fetch from the mirror specified on the configuration file.""",
            usage=f"{sys.argv[0]} dl-iso [OPT]",
        )
        parser.add_argument(
            "-u", help="specify the root mirror URL to use", dest="mirror_url", type=str
        )
        parser.add_argument(
            "-m",
            help="get the metadata of the remote ISO",
            action="store_true",
        )
        parser.add_argument(
            "--json", help="print output in JSON format", action="store_true"
        ...
        if not os.path.exists(self.config_content["paths"].get("container_iso_path")):
            self.__create_file_recursively(
                self.config_content["paths"].get("container_iso_path")
            )

        if args.mirror_url:
            url_check = urlparse(args.mirror_url)
            if not url_check.geturl():
                raise ValueError(f"{args.mirror_url} is not a valid URL format")

            mirror_url = url_check.geturl()

        remote_iso_manager = RemoteISOManager(
            self.config_content["paths"].get("permalink_resource_file_url"),
            self.config_content["paths"].get("container_iso_mirror_url")
            if not mirror_url
            else mirror_url,
        )

        if not remote_iso_manager.getContainerISOMirrorURL():
            self.__log(
                message="Fetching permalink remote ISO mirror URL ...", bypass=args.json
            )

            remote_iso_manager.setContainerISOMirrorURL(
                remote_iso_manager.getPermalinkResourceFileURLContent()
            )

        self.__log(message="Fetching remote ISO image metadata ...", bypass=args.json)

        checksums = remote_iso_manager.getRemoteISOChecksums()
        version = remote_iso_manager.getRemoteISOVersion()
        ...
        if args.m:
            if args.json:
                self.__log_json(
                    ...
                    "Remote ISO metadata",
                    data={
                        "container_iso_mirror_url": self.config_content["paths"].get(
                ...
                self.__log(
                    LOG_START_SUCCESS,
                    f"Remote ISO metadata ({self.config_content['paths'].get('container_iso_mirror_url') if not mirror_url else mirror_url}):",
                )
                self.__log(
                    message=f"Checksum : {checksums[1]}",
                    tabulation=True,
                )
                self.__log(
        ...
        self.__log(
            message=f"Downloading ISO image from {self.config_content['paths'].get('container_iso_mirror_url') if not mirror_url else mirror_url} ...",
            bypass=args.json,
        )

        remote_iso_manager.downloadRemoteISO(
            self.config_content["paths"].get("container_iso_path")
        )

        if (
            remote_iso_manager.makeISOChecksum(
                self.config_content["paths"].get("container_iso_path")
            )
            != checksums[1]
        ):
            raise RuntimeError("Checksum mismatch for downloaded ISO")

        if args.json:
            self.__log_json(
                LOG_JSON_STATUS_SUCCESS,
                "ISO image was successfully downloaded",
                data={
                    "checksums": checksums,
                    "version": version,
                    "file_path": self.config_content["paths"].get("container_iso_path"),
                },
            )
            return

        self.__log(LOG_START_SUCCESS, "ISO image was successfully downloaded")
        self.__log(
            message=f"Checksum : {checksums[1]}",
            tabulation=True,
        )
        ...
        self.__log(
            message=f"File was stored in {self.config_content['paths'].get('container_iso_path')}\n",
            tabulation=True,
        )
